...
public class GeneradorClientes {
    ...
    public static void generarArchivo(String ruta, int cantidad) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(ruta))) {
            ...
            for (int i = 1; i <= cantidad; i++) {
                ...
                writer.write(i + "," + "Cliente_" + i + "," + ingreso + "," + segmento + "," + region + "," + score + "," + deuda + ",\"" + jsonData.replace("\"", "'") + "\"");
                writer.newLine();
            }
    ...
    private static String generarJsonPesado(Random random) {
        ...
        sb.append("\"historial\":[");
        for (int i = 0; i < 10; i++) {
            sb.append("{\"evento\":\"evento_").append(i).append("\",\"monto\":").append(random.nextInt(5000)).append(",\"estado\":\"ACTIVO\"}");
            if (i < 9) sb.append(",");
        }
